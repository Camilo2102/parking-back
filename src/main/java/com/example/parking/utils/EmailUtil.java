...
import com.example.parking.models.utils.Email;
...
public class EmailUtil {
    ...
    private static String parkingEmail;
    private static String parkingPassword;
    ...
    private void setCredential(Environment env){
        parkingEmail = env.getProperty("parking.email");
        parkingPassword = env.getProperty("parking.password");
    }
    ...
    public static void initializeEmailUtil(){
        ...
        mProperties.setProperty("mail.smtp.user", parkingEmail);
    ...
    public static void sendEmail(Email email){
        Thread emailThread = new Thread(()-> {
            try {
                ...
                mTransport.connect(parkingEmail, parkingPassword);
                mCorreo = new MimeMessage(mSession);
                mCorreo.setFrom(new InternetAddress(parkingEmail));
    ...
    /**
     * OBtiene el parking a enviar en el correo de registro
     ...
     */
    public static String getUserRegisterParking(String token, String code) {
    ...
    /**
     * OBtiene el parking a enviar en el correo de recuperacion
     * @param token el token que se necesita en la url
     * @param code el codigo a mostrar al usuario
     * @return un string con el codigo html para enviar al correo
     */
    public static String getUserRecoveryParking(String token, String code) {
     ...
     */
    public static String sendPasswordChangeMail(ParkingUser user, String mail, String code) {
        Map<String, String> tokenData = new HashMap<>();
        tokenData.put("id", user.getId());

        String temporalToken = TokenUtil.generateToken(tokenData, 60 * 24);

        Email email = new Email(mail, "Completa tu registro", EmailUtil.getUserRegisterParking(temporalToken, code));
     ...
     * @return el tokentemporal generado
     */
    public static String sendPasswordChangeMail(ParkingUser user, String mail, String code, boolean recovery) {
        Map<String, String> tokenData = new HashMap<>();
        tokenData.put("id", user.getId());

        String temporalToken = TokenUtil.generateToken(tokenData, 60 * 24);

        Email email = new Email(mail, "Recupera tu contraseña: ", EmailUtil.getUserRecoveryParking(temporalToken, code));
        EmailUtil.sendEmail(email);
